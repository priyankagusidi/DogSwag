     ...
     <Head>
          <title>DogSwag - For every dogs swag, Blog about dogs, share and get paid</title>
            ...
            <div className="w-full flex flex-col relative">
                 <Doctor doctordata={props.doctordata} userdata={props.userdata} isEditing={false} button_title={true}/>
        </div>
      </div>
    )
}
...
try{
    ...
    if(!userdata.user.isDoctor){
     ...
     const res2 = await fetch(`${url}/api/doctor/profile/one`,{
        withCredentials: true,
        headers: {
            Cookie: req.headers.cookie
        }
     ...
     const doctordata = await res2.json()
