...
import CreateBlog from '@/components/write'
import Head from 'next/head'
import dynamic from 'next/dynamic'
// const Tip = dynamic(() => import("@/components/write/tip"), {
//   ssr: false,
// });
...
const index = (props) => {
  ...
  return (
    <div className="bg-gray-100 h-fit">
      ...
      {/*<Tip/>*/}
      <CreateBlog userprofiledata={props.userprofiledata} blogdata={props.blogdata}/>
...
export async function getServerSideProps({req,params}) {
   try{
    ...
    // const res5 = await fetch(`${url}/api/`)
    const res5 =await fetch(`${url}/api/blogs/blog/${slug}`)
    const blogdata = await res5.json()
    ...
    return {
        props : {userdata,userprofiledata ,notificationdata,blogdata}
     }
