...
import DogProfile from '@/components/pubdogprofile';
...
const index = (props) => {
    return (
      ...
      <DogProfile dogdata={props.dogdata} videodata={props.videodata}/>
      </div>
    );
};
...
try{
    ...
    if(!userdata.user){
     ...
     const res2 = await fetch(`${url}/api/doginfo/profile/one/${id}`, {
        withCredentials: true,
        headers: {
            Cookie: req.headers.cookie
        }
     ...
     const res3 = await fetch(`${url}/api/video/all`, {
        withCredentials: true,
        headers: {
            Cookie: req.headers.cookie
        }
     ...
     return {
        props : {userdata,dogdata,videodata}
     }
      }catch(err){
         return {
