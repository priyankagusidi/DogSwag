...
export default function DogModal({fetchDogProfile,dog,isDogModalOpen,setIsDogModalOpen,type}){
  ...
  const [info, setinfo] = useState(dog ? dog : {dogImage:"/doguser.png",dogName:"milo",dogGender:"Male",dogBreed:"German",dogAge:"8",dogWeight:"12"});
	...
	async function handleEdit(){
		...
		const formData = new FormData();
		    formData.append('profilePic',info.dogImage)
	...
	return(
		  ...
		  <div className="grid grid-cols-10 gap-2 text-sm">
		    ...
		    <label className="flex justify-start border col-span-3">
		  	...
		  	<select onChange={handleChange} value={info?.dogAge} name="dogAge"  className="p-2 border bg-gray-100 rounded-lg col-span-7">
		  	  {
		  	  	age.fill(50).map((w,i)=>{
		  	  		return <option key={i} className="p-2">{i+1}</option>
		  	  	})
		  	  }
		  	</select>


		  	{/*<input  className="p-2 border bg-gray-100 rounded-lg col-span-7" placeholder="Age"/>*/}
        
        <label className="flex justify-start border col-span-3">
		  	  <h1>Gender</h1>
		  	</label>
		  	<select onChange={handleChange} value={info?.dogGender}  name="dogGender" className="p-2 border bg-gray-100 rounded-lg col-span-7">
		  	  <option className="p-2">Male</option>
		  	  <option className="p-2">Female</option>		  	  
		  	</select>
        ...
        <label className="flex justify-start border col-span-3">
		  	...
		  	<select onChange={handleChange} value={info?.dogWeight}  name="dogWeight" className="p-2 border bg-gray-100 rounded-lg col-span-7">
		  	  {
		  	  	weight.fill(50).map((w,i)=>{
		  	  		return <option key={i} className="p-2">{i+1}</option>
		  	  	})
		  	  }
		  	</select>
