...
const PricingPage = ({userdata,pricingModal,setPricingModal,billingdata,pricingRef}) => {
 ...
 const handleOpenWindow = (data) => {
    const newWindow = window.open('', '');
    newWindow.document.write(data); // Write the HTML content to the new window
  };
  ...
  const handlePayment = async (amount,currency,plantype,blogsCount,notificationsCount) => {
     ...
     try {
      const {data} = await axios.post('/api/billing/create',{
         amount, currency:"INR",plantype,blogsCount,notificationsCount
      })
      document.open(); 
      document.write(data); 
      document.close(); 
...
const topUps = [
 ...
 const fetchCountry = async () => {
  ...
  return (
    ...
    >  
    <div className="">
      <h1 className="pb-5 text-center text-xl font-bold ">Choose a Plan</h1>
     {
     billingdata && billingdata.trialQuotaAlert?
      <p className="text-center font-bold text-sm text-red-500 mb-5">
         Quota of 10 notification has ended,<br/>
         Buy the Swag pack within 30 days to keep the Account and Data active.
      </p>:
     billingdata && billingdata.premiumQuotaAlert?
      <p className="text-center font-bold text-sm text-red-500 mb-5">
         Quota of 150 notification has ended,<br/>
         Buy the Top Ups to get more notifications.
      </p>:
      <></>
     }
      {
...

    {
      topUps.map((m,i)=>{
        return(
   <div class="card-wrapper shadow-2xl" key={m.id}> 
    ...
    <div key={i} class="w-full shadow-2xl text-xs px-6 py-4 transition-colors duration-300 transform rounded-lg md:mx-5 md:w-96 bg-gray-50 dark:bg-gray-800">
           ...
           m.benefits.map((k,i)=>{
             return (
               <div key={k.id} class="flex items-center">
                ...
                <button id="select_price_button" onClick={()=>openBillingModal(m.amount,m.plantype,m.notificationcount,m.blogcount,m.benefits)} class="w-full px-4 py-2 mt-10 font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-blue-500 rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600">
                    I WANT IT
                </button>
            </div>
          )
   ...
   <CalenderBillingModal hashValue={hashValue} setHashValue={setHashValue} handlePayment={handlePayment} error={error} benefits={benefits} country={country} billingModal={billingModal} plantype={plantype} notificationsCount={notificationsCount} blogsCount={blogsCount} transactionIdValue={transactionIdValue} amount={amount} setBillingModal ={setBillingModal}/>
  </Modal>
  )
}
