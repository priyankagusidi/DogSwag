...
export default function Index(){
	...
	async function SendPassword(){
		setMessage("")
		try{
			await axios.post(`/api/auth/resetemail`,info).then(res=>{
	    ...
	    <div className="flex items-center  absolute  top-2">
	      ...
	      <span className="text-[#5F2600] font-bold text-xl">DogSwag</span>
		   ...
		   <div className="max-w-md mt-20 h-fit mx-auto rounded-xl bg-white flex flex-col gap-2 w-full shadow-lg">
		    ...
		    <h2 className="text-2xl font-semibold">Enter your neew password</h2>
		    <input onChange={handleChange} name={"password"} type="password" placeholder="password" className="p-2 border border-gray-300 w-full"/>
		    <input onChange={handleChange} name={"confirmpassword"} type="password" placeholder="confirmpassword" className="p-2 border border-gray-300 w-full"/>
		    <button onClick={SendPassword} className="p-2 bg-amber-800 rounded-xl text-white">Reset password</button>		     
            </div>
		    </div>
